...
`~/.t3`. Agents use the account's Git config, agent logins, and shared skills.
...
Sign in to Claude Code and OpenCode on each host:

```sh
ssh -t adam 'claude auth login'
ssh -t adam 'opencode auth login'
ssh -t lilith 'claude auth login'
ssh -t lilith 'opencode auth login'
```
