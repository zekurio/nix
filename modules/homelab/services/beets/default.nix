{
  flake.modules.nixos.homelab = {
    ...
    cleanImportLeftovers = pkgs.writeShellApplication {
      name = "clean-import-leftovers";
      text = ''
        exec ${lib.getExe pkgs.python3} ${./clean-import-leftovers.py} "$@"
      '';
    };
    ...
    beetsConfig = (pkgs.formats.yaml {}).generate "beets-config.yaml" {
      ...
      # Resized artwork can retain tempfile mode 0600, hiding it from media players.
      permissions = {
        file = "664";
        dir = "2775";
      };
    ...
    beetsImportWorker = pkgs.writeShellApplication {
      ...
      text = ''
        ...
        trap 'rm -f "$scan_boundary" "$cleanup_manifest"' EXIT
        ...
        ${lib.getExe cleanImportLeftovers} record "''${import_dirs[@]}" > "$cleanup_manifest"
        ...
        if ${lib.getExe beetInternal} import -I -m -q --from-scratch "''${candidates[@]}"; then
          ...
          ${lib.getExe cleanImportLeftovers} clean "$cleanup_manifest"
          ${lib.getExe cleanImportLeftovers} prune "''${import_dirs[@]}"
