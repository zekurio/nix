/**
 * Vendored from mitsuhiko/agent-stuff (Apache-2.0), extensions/btw.ts.
...
export default function (pi: ExtensionAPI) {
	...
	async function createSideSession(ctx: ExtensionCommandContext): Promise<SideSessionRuntime | null> {
		...
		const { session } = await createAgentSession({
			...
			// This session uses an isolated runtime, so do not give it tools that
			// could bypass the parent session's permission and path safeguards.
			tools: [],
