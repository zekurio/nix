...
in {
  opencode = prev.stdenvNoCC.mkDerivation (finalAttrs: {
    ...
    patches = [
      ./opencode-relax-bun-version-check.patch
    ];

    dontConfigure = true;

    env.MODELS_DEV_API_JSON = "${prev.models-dev}/dist/_api.json";
    ...
    installPhase = ''
      ...
      mkdir -p $out/bin
      makeWrapper ${prev.lib.getExe prev.bun} $out/bin/opencode \
        --add-flags "run" \
    ...
    passthru = {
      ...
      tests.version = prev.testers.testVersion {
    ...
    meta = {
      ...
      maintainers = with prev.lib.maintainers; [delafthi];
      sourceProvenance = with prev.lib.sourceTypes; [fromSource];
      platforms = [
        "aarch64-linux"
        "x86_64-linux"
        "aarch64-darwin"
