...
- `modules/hosts/`: host-specific NixOS systems such as `adam/` and `tabris/`
...
- `nix build .#nixosConfigurations.tabris.config.system.build.toplevel`: build the WSL host config
- `sudo nixos-rebuild switch --flake .#tabris`: apply the local WSL configuration
...
Recent history mixes short fixes (`fix DNS`, `fix beets`) with conventional commits (`feat(adam): ...`, `chore: update flake.lock`). Prefer imperative, scoped subjects such as `feat(tabris): enable service` or `fix(adam): correct DNS`. Keep commits focused.
